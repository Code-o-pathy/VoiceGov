...
import { PAN_REGEX, normalizePan } from "@/lib/replica/mockApi";
...
export function parseIntentLocal(utteranceRaw: string): Intent {
  ...
  const isRefund = REFUND_MARKERS.some((m) => lower.includes(m));
  if (isRefund) {
    return {
...
export function extractEntities(utterance: string): Record<string, string> {
  ...
  const panMatch = utterance
    .toUpperCase()
    .match(/[A-Z]{5}\s?[0-9]{4}\s?[A-Z]/);
  if (panMatch) {
    const pan = normalizePan(panMatch[0]);
    if (PAN_REGEX.test(pan)) entities.pan = pan;
  }

  // Assessment year: "2025-26", "2025 26", "AY 2024", or a bare year.
